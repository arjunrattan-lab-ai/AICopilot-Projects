...
- `../pm-references/jira-api.md` — direct Jira REST API patterns (use instead of MCP for reads)
...
- **Parent page:** TSSD Triage Briefs (ID `6282412041`)
- **Page title convention:** `TSSD / {ticket-key}: {short summary}`
</confluence>
...
Read the full ticket using direct Jira API (see `../pm-references/jira-api.md`):
```bash
source /Users/sumit.suman/Documents/00\ ai-projects/.env
curl -s -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_TOKEN}" \
  "https://k2labs.atlassian.net/rest/api/3/issue/{ticket-key}?fields=summary,status,priority,description,issuetype,labels,created,updated,assignee,reporter,comment" \
  | python3 -c "
import json, sys
...
def extract_text(node):
    if isinstance(node, dict):
        t = node.get('text', '')
        return t + ''.join(extract_text(c) for c in node.get('content', []))
    return ''
print(f'Key: {d[\"key\"]}')
...
This returns ~20 KB instead of ~35 KB from MCP by requesting only needed fields.
...
1. **Related TSSD tickets:** Search for similar tickets (same labels, same component, same
   ...
   source /Users/sumit.suman/Documents/00\ ai-projects/.env
   curl -s -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_TOKEN}" \
     "https://k2labs.atlassian.net/rest/api/3/search/jql?jql={URL_ENCODED_JQL}&maxResults=15&fields=summary,status,priority,labels,created,assignee" \
     | python3 -c "import json,sys; d=json.load(sys.stdin); [print(f'{i[\"key\"]:12s} {i[\"fields\"][\"status\"][\"name\"]:15s} {i[\"fields\"][\"priority\"][\"name\"]:10s} {i[\"fields\"][\"summary\"][:80]}') for i in d.get('issues',[])]"
   ```
...
2. **Engineering tickets:** Search for related engineering Jira issues (linked issues,
   ...
   Same curl pattern with JQL: `text ~ "{key terms}" AND project not in (TSSD) AND created >= -90d ORDER BY created DESC`
...
5. **Related tickets (broader net):** Go beyond the initial TSSD search. Pull:
   ...
   Same curl pattern with JQL: `issue in linkedIssues({ticket-key}) ORDER BY created DESC`
   Same curl pattern with JQL: `reporter = {reporter_account_id} AND created >= -180d ORDER BY created DESC`
