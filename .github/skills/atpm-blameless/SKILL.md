...
- **Parent page:** Product Incident Reviews (ID `6283591691`)
- **Page title convention:** `PIR / {short title}`
- **Labels:** `pir`, `blameless`
- **Diagrams:** mermaid.ink inline image URLs (see confluence-sync.md § Mermaid Diagrams)
</confluence>
...
description, a reference to a known event. If it is a ticket, read it using direct Jira API (see `../pm-references/jira-api.md`).
Use the single-issue curl pattern with `fields=summary,status,priority,description,issuetype,labels,created,updated,assignee,reporter,comment`.
If it is free text, search for every ticket that touches it using the search curl pattern.
...
Use direct Jira API for all searches (see `../pm-references/jira-api.md`). Use the search
curl pattern with `fields=summary,status,priority,labels,created,assignee` for scan queries,
and the single-issue pattern with full fields for tickets you need to read deeply.

Search JQLs:
- Linked issues: `issue in linkedIssues({ticket-key}) ORDER BY created ASC`
- Keyword search: `text ~ "{key terms}" AND created >= -{window}d ORDER BY created ASC`
- TSSD symptom: `project = TSSD AND text ~ "{symptom}" AND created >= -{window}d ORDER BY created ASC`
...
Use direct API search with JQL: `text ~ "{failure pattern}" AND created >= -180d ORDER BY created DESC`
