# PDP: AIOC+ Pedestrian Warning (Waste Residential + Urban Transit V1)

> **Scope note (2026-04-21):** Construction / aggregates is **out of scope** for V1 (no V2 commit). In-cab display cannot be mounted usably across heterogeneous construction vehicle cabs. V1 targets waste residential pickup (backing) and urban transit/delivery (right-turn). See STATE.md → Scope Decisions.
...
V1 targets **waste residential pickup (backing)** and **urban transit/delivery (right-turn)** — the segments where customer signal is validated and in-cab display mounting is feasible. Anchor deal is Waste Connections 13k ("co-develop native ped detection"). GreenWaste has signaled that side-camera AI is a "must" to displace 3rdEye. Ace Disposal (closed-won) sets the public precision benchmark (FPs on bollards, cones, mailboxes). ([PROBLEM.md — Customer Validation](pm-planning/aioc+-pedestrian-warning/PROBLEM.md))

---

## 2. Problem

Fleet operators in waste and transit segments cannot detect pedestrians in side and rear blind spots during low-speed maneuvers. Drivers physically cannot see workers, pedestrians, or cyclists behind or beside their vehicle when backing, turning, or idling in yards.
...
- Credible SAM for ped-sensitive segments after the 2026-04-21 construction-out-of-scope decision is waste + transit only. The $25M figure from concept slides is overstated: Republic uses Samsara, WM uses Lytx, Transdev has no deal signal. ([PROBLEM.md — Impact](pm-planning/aioc+-pedestrian-warning/PROBLEM.md))
...
|---|---|---|---|
| Waste Connections 13k (anchor) | Waste | "Motive Product has proposed WC co-develop our native ped detection offering" | SFDC (Michael Alexander), Apr 2026 |
| Ace Disposal | Waste | FPs on bollards, cones, mailboxes = public precision benchmark | Provision research / SFDC |
...
| FirstFleet / Werner | Freight / transit-adjacent | "Must have prevention" + blinker-triggered side BSM explicit | SFDC (William Tafel) |
...
| AIOC+ deals citing ped detection as buying criterion | 1 (WC 13k anchor, explicit) | 3 named prospects | 5+ | AE survey / CRM |
...
| Decision | Rationale |
|---|---|
| **V1 = stopped/yard-nudge + reverse backing (2 use cases)** | Validated customer signal in waste residential pickup (Waste Connections anchor, Ace Disposal benchmark, GreenWaste displacement). Tightest AI definition (near-field, no TTC, no turn detection). Simplest state machine. Version-gated per Gautam Kunapuli's constraint. ([SOLUTION.md — Why UC1 + UC2](pm-planning/aioc+-pedestrian-warning/SOLUTION.md)) |
...
| Customer validation returns negative (no one pays for this) | Medium | High | Do not commit full eng resources until AE interviews (WC, Ace, GreenWaste, Alliance, FirstFleet, Academy, NYC School Bus) return. See `scratch/ae-customer-target-list.md`. | Arjun Rattan (PM) |
...
|---|---|---|---|
| 1 | AE validation: Is ped detection a buying criterion for Waste Connections, Ace Disposal, GreenWaste, Alliance Environmental? Buyer, scenario, precision bar, ROI framing. | Arjun Rattan (AE interviews) | Before Alpha (Jul 2026) |
| 2 | AE validation: Right-turn signal — does FirstFleet/Werner, Academy Bus, NYC School Bus confirm the use case? | Arjun Rattan (AE interviews) | Before Alpha |
